...
const WarningMessageModal = forwardRef((props, ref) => {
  let [isOpen, setIsOpen] = useState(false);
  const cancelButtonRef = useRef(null);

  const closeModal = (result) => {
    setIsOpen(false);
    props.WarningResultCallback(result);
  };
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  ...
                  <button
                    type="button"
                    className="bg-gray-200 inline-flex w-full justify-center rounded-md px-3 py-2 text-sm font-semibold  shadow-sm hover:bg-gray-400 sm:ml-3 sm:w-auto"
